Here's an expanded version of the documentation that includes all manual values and methods for each type of visualization, broken down by **location type** and **crop group**:
...
#### 2. Calculate Yield per Hectare:
   - Add a derived column, `yield_per_ha`, to calculate productivity using the formula:
     \[
     \text{Yield per Hectare} = \frac{\text{Actual Quantity}}{\text{Location Area (ha)}}
     \]

#### 3. Initial Outlier Detection Using IQR:
   - Compute the Interquartile Range (IQR) for `actual_quantity` within each `crop_group`:
     \[
     \text{IQR} = Q3 - Q1
     \]
