...
#### 6. Visualization-Specific Outlier Handling for Dashboards

When generating dashboard visualizations involving area-related variables—such as farm_area, productive_area, and similar metrics—we apply dynamic outlier filtering based on country and date range. This ensures users receive clean, representative data even in visual summaries.

#### Process:

##### Country and Date Filtering:

-   Before plotting, we filter the data based on the selected country and created_at date range.
...
-   Q1 and Q3 are recalculated for the area variables.
